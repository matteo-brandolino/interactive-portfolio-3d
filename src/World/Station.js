...
export default class Station {
    ...
    createMarker() {
        ...
        if (woodenSignModel) {
            ...
            console.warn('Wooden sign model not found, using fallback')
    ...
    createLabel() {
        ...
        context.fillText(`${this.data.icon} ${this.data.title}`, canvas.width / 2, canvas.height / 2)
    ...
    startFloatingAnimation() {
        if (this.marker) {
            const targetY = this.marker.position.y + 0.1
            gsap.to(this.marker.position, {
                y: targetY,
                duration: 3,
                repeat: -1,
                yoyo: true,
    ...
    setActive(active) {
        ...
        if (active) {
            ...
            console.log(`📍 Station active: ${this.data.title}`)
        } else {
            if (this.label && this.label.material) {
                gsap.to(this.label.material, {
                    opacity: 0,
